...
void aiproporcional() { // izquierda
  ...
  // rueda atrás apoya corrección
    ...
    digitalWrite(INA1, LOW); digitalWrite(INB1, HIGH); analogWrite(PWM1, pwmM);
...
void adproporcional(){
  ...
  digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW); analogWrite(PWM1, pwmM);
 ...
 currentMillis = millis();
  ...
  switch (estado1) {
    ...
    if (verificarPelotaVioleta()){
     ...
     if (currentRoll > 3.5 || currentMillis - previousMillis >= 2000 ){
      pare();
      /*estado1 = 2;*/
      previousMillis = currentMillis;
     }
  ...
  if (correccion < -MAX_CORR) correccion = -MAX_CORR;
    ...
    case 8:
      adproporcional();
      if(switchDerechoActivado || (currentMillis - previousMillis) >= 250){
        pare();
      estado1 = 9;
      previousMillis = currentMillis; 
      }
    ...
    if (verificarPelotaVioleta()){
        ggi();
        if ((currentMillis - previousMillis) >= 450)
        ggd();
        estado1 = 0;
      }
  ...
  if (correccion < -MAX_CORR) correccion = -MAX_CORR;
    ...
    case 0: 
    adproporcional();
    if(switchDerechoActivado || (currentMillis - previousMillis) >= 2400){
      pare();
        esquina = 1;
        previousMillis = currentMillis;
        }
