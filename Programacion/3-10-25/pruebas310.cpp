...
void rproporcional(float correccion) { // retroceder corrigiendo
...
void loop() {
 ...
 currentMillis = millis();
  ...
  case 0: {
    ...
    float kd = (error - erroranterior);
    float kt = (currentMillis - previousMillis);
    float kf = (kd / kt);
    float eForCtrl = (fabs(error) < DEADBAND) ? 0.0f : error;

    correccion = eForCtrl * kpatras + kdc3 * kf;
    if (correccion > MAX_CORR) correccion = MAX_CORR;
    if (correccion < -MAX_CORR) correccion = -MAX_CORR;
