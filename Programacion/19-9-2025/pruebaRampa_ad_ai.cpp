...
// Velocidad base lateral para AI/AD
int velocidad_lateral = 40;
int velocidad_lateral_trasera = 87;
...
void aiproporcional() { // izquierda proporcional con corrección de rumbo
  int pwmM3 = clampPWM(velocidad_lateral - (int)correccion); // M3 adelante
  int pwmM2 = clampPWM(velocidad_lateral + (int)correccion); // M2 adelante
  ...
  digitalWrite(INA1, LOW);  digitalWrite(INB1, HIGH); analogWrite(PWM1, velocidad_lateral_trasera);
}

void adproporcional() { // derecha proporcional con corrección de rumbo
  int pwmM3 = clampPWM(velocidad_lateral + (int)correccion); // M3 atrás
  int pwmM2 = clampPWM(velocidad_lateral - (int)correccion); // M2 atrás

  digitalWrite(INA3, LOW);  digitalWrite(INB3, HIGH); analogWrite(PWM3, pwmM3);
  digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, pwmM2);

  // Rueda trasera mantiene lateral fijo
  digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW);  analogWrite(PWM1, velocidad_lateral_trasera);
}
...
void loop() {
 ...
 currentMillis = millis();
  ...
  switch (estado1) {
    ...
    if(error > 15){
           digitalWrite(INA3, LOW); digitalWrite(INB3, HIGH);  analogWrite(PWM3, 0);
           digitalWrite(INA2, HIGH);  digitalWrite(INB2, LOW); analogWrite(PWM2, 70);
           digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW); analogWrite(PWM1, 70);
      }
    ...
    case 8:
      adproporcional();
      if(currentMillis - previousMillis >= 550){
        pare();
      estado1 = 9;
      previousMillis = currentMillis; 
      }
