...
void aiproporcional() { // izquierda proporcional con corrección de rumbo
  int pwmM3 = clampPWM(velocidad_lateralai - (int)correccion); // M3 adelante
  int pwmM2 = clampPWM(velocidad_lateralai + (int)correccion); // M2 adelante

  digitalWrite(INA3, HIGH); digitalWrite(INB3, LOW);  analogWrite(PWM3, pwmM3);
  digitalWrite(INA2, HIGH); digitalWrite(INB2, LOW);  analogWrite(PWM2, pwmM2);

  // Rueda trasera mantiene lateral fijo
  digitalWrite(INA1, LOW);  digitalWrite(INB1, HIGH); analogWrite(PWM1, velocidad_lateral_traseraai);
}

void adproporcional() { // derecha proporcional con corrección de rumbo
  int pwmM3 = clampPWM(velocidad_lateralad + (int)correccion); // M3 atrás
  int pwmM2 = clampPWM(velocidad_lateralad - (int)correccion); // M2 atrás
  ...
  // Rueda trasera mantiene lateral fijo
  digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW);  analogWrite(PWM1, velocidad_lateral_traseraad);
}
