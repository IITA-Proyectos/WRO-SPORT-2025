...
// Ajustables para calibración en pista
const float CHOQUE_UMBRAL_MSS = 2.3f;         // m/s^2 (magnitud de aceleración lineal)
const unsigned long CHOQUE_DEBOUNCE_MS = 60;  // persistencia mínima del pico
const unsigned long CHOQUE_COOLDOWN_MS = 400; // evitar dobles detecciones
const float CHOQUE_LPF_ALPHA = 0.25f;         // filtro paso bajo simple (0..1)
const unsigned long CHOQUE_IGNORE_START_MS = 250; // ignorar picos al arrancar avance

// Eje frontal del robot respecto al frame del BNO055
// 0=X, 1=Y, 2=Z. SIGN=+1 si el eje del sensor apunta hacia adelante del robot, -1 si al revés
const int ACCEL_FORWARD_AXIS = 0; // ajustar según montaje físico del BNO055
const int ACCEL_FORWARD_SIGN = +1; // ajustar según orientación
...
static inline bool detectarChoque() {
  // Aceleración lineal sin gravedad del BNO055
  ...
  // Ignorar picos al inicio del avance
  if (now < choqueIgnorarHastaMs) {
    choqueInicioMs = 0;
    return false;
  }

  // Proyección direccional sobre el eje frontal del robot
  ...
  // Considerar choque solo si hay desaceleración frontal (negativa) sostenida por encima del umbral
  ...
  switch (estado1) {
    ...
    case 1: // avanza por tiempo con corrección + freno por choque
      ...
      if (detectarChoque()) {
        pare();
        previousMillis = currentMillis;
        estado1 = 2; // continúa la rutina como si hubiera terminado el avance
      } else if (currentMillis - previousMillis >= 1500) { // depende de batería
        previousMillis = currentMillis;
        choqueIgnorarHastaMs = millis() + CHOQUE_IGNORE_START_MS; // ignorar pico de arranque
        estado1 = 2;
      break;
