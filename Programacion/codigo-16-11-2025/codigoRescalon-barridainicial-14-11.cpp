...
//codigo para la competencia de panama
//robot escalon con deteccion de choque y camara para pelotas moradas

//codigo modificado el 14/11
#include <arduino.h>
#include <Wire.h>
...
//int estado = 0;
int estado1 = 15; //aclaracion: con el codigo del 14/11 0 comienza la rutina en bucle y 15 barre las pelotas
...
void a() { // avanzar recto “rápido” sin corrección
  digitalWrite(INA3, LOW); digitalWrite(INB3, HIGH); analogWrite(PWM3, 30);
...
void aproporcional3() { // avanza corrigiendo
  int pwmL = clampPWM(velocidad_adelante - (int)correccion3);
  int pwmR = clampPWM(velocidad_adelante + (int)correccion3);
...
void ggd(){
  digitalWrite(INA1, LOW);  digitalWrite(INB1, HIGH); analogWrite(PWM1, 30);
  digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, 0);
  digitalWrite(INA3, LOW);  digitalWrite(INB3, HIGH); analogWrite(PWM3, 30);
}
void ggi(){
  digitalWrite(INA1, HIGH);  digitalWrite(INB1, LOW); analogWrite(PWM1, 30);
  digitalWrite(INA2, HIGH);  digitalWrite(INB2, LOW); analogWrite(PWM2, 30);
...
void loop() {
  ...
  correccion2 = eForCtrl2 * kpdetras;
 ...
 currentMillis = millis();
  ...
  switch (estado1) {
   ...
   case 16:
    ...
    if (currentMillis - previousMillis >= 500) { //depende la bateria va a cambiar la velocidad y el tiempo
      pare();
       previousMillis = currentMillis;
       estado1 = 17;
    }
