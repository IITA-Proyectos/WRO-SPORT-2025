//codigo robot escalon con rutina al inicio de barrido en el caso 15
//definitivo ultimo provado en salta
//codigo modificado el 21/11
#include <arduino.h>
#include <Wire.h>
...
float initialRoll = 0;
int velocidad_adelante = 80; //antes era 70
int velocidad_adelante2 = 40;
int velocidad_atras = 60; // antes era 80
int velocidad_izquierda = 40;
int velocidad_medio = 69;
int velocidad_derecha = 40;//--->22-9-2025 estaba en 40, pero no se movia a la derecha casi.
int correccion_motor_derecho = 0;
int correccion_motor_izquierdo = 0;
float kpadelante = 3.0f; //antes era 10.0f
float kpdetras = 1.8f;
float kpadelante2 = 6.0f;
...
//int estado = 0;
int estado1 = 17; //aclaracion: con el codigo del 14/11 0 comienza la rutina en bucle y 15 barre las pelotas


// Variables para cámara - parser no bloqueante kp
...
void aproporcional2() { // avanza corrigiendo
  int pwmL = clampPWM(velocidad_adelante - (int)correccion3);
  int pwmR = clampPWM(velocidad_adelante + (int)correccion3);

  // motor izquierdo (M3) en reversa
  digitalWrite(INA3, LOW); digitalWrite(INB3, HIGH); analogWrite(PWM3, pwmL - correccion_motor_izquierdo);
  // motor derecho (M2) adelante
  digitalWrite(INA2, HIGH); digitalWrite(INB2, LOW);  analogWrite(PWM2, pwmR);
...
void aproporcional4() { // avanza corrigiendo
  int pwmL = clampPWM(velocidad_adelante2 - (int)correccion4);
  int pwmR = clampPWM(velocidad_adelante2 + (int)correccion4);
...
void ai() { // izquierda
  digitalWrite(INA3, HIGH); digitalWrite(INB3, LOW);  analogWrite(PWM3, 30);
  digitalWrite(INA2, HIGH); digitalWrite(INB2, LOW);  analogWrite(PWM2, 30);
  digitalWrite(INA1, LOW);  digitalWrite(INB1, HIGH); analogWrite(PWM1, 60);
}
...
void aiproporcional() { // izquierda
  ...
  int pwmM = clampPWM(velocidad_medio + (int)correccion);// agregado el 20-10-2025
  ...
  // rueda atrás apoya corrección
    ...
    digitalWrite(INA1, LOW); digitalWrite(INB1, HIGH); analogWrite(PWM1, pwmM);//modificado el 20-10-2025 antes era 70
...
void adproporcional(){
  ...
  // rueda atrás apoya corrección
    ...
    digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW); analogWrite(PWM1, pwmM);//modificado el 20-10-2025 antes era 70

}

void rproporcional() { // retroceder corrigiendo
  int pwmL = clampPWM(velocidad_atras + (int)correccion2);
  int pwmR = clampPWM(velocidad_atras - (int)correccion2);

  // motor izquierdo (M3) adelante
  digitalWrite(INA3, HIGH); digitalWrite(INB3, LOW);  analogWrite(PWM3, pwmL);
  // motor derecho (M2) atrás
  digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, pwmR + correccion_motor_derecho);
...
void r() { // retro recto sin corrección (se usa poco)
  digitalWrite(INA3, HIGH); digitalWrite(INB3, LOW);  analogWrite(PWM3, 200);
  digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, 200);
  digitalWrite(INA1, LOW);  digitalWrite(INB1, LOW);  analogWrite(PWM1, 0);
}

void gi (){ // giro derecha
...
void rd (){ // giro derecha
  digitalWrite(INA1, HIGH);  digitalWrite(INB1, LOW); analogWrite(PWM1, 50);
  digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, 50);
...
void loop() {
  ...
  correccion = eForCtrl * kpadelante;
  if (correccion >  MAX_CORR) correccion =  MAX_CORR;
  if (correccion < -MAX_CORR) correccion = -MAX_CORR;

  float eForCtrl2 = (fabs(error) < DEADBAND) ? 0.0f : error;
  correccion2 = (eForCtrl2 - 5) * kpdetras;
  if (correccion2 >  MAX_CORR) correccion2 =  MAX_CORR;
  if (correccion2 < -MAX_CORR) correccion2 = -MAX_CORR;

  float eForCtrl3 = (fabs(error) < DEADBAND) ? 0.0f : error;
  correccion3 = (eForCtrl3 - 5) * kpadelante2;
  if (correccion3 >  MAX_CORR) correccion3 =  MAX_CORR;
  if (correccion3 < -MAX_CORR) correccion3 = -MAX_CORR;

  float eForCtrl4 = (fabs(error) < DEADBAND) ? 0.0f : error;
  correccion4 = eForCtrl4 * kpadelante;
  if (correccion4 >  MAX_CORR) correccion4 =  MAX_CORR;
  if (correccion4 < -MAX_CORR) correccion4 = -MAX_CORR;
 currentMillis = millis();
  ...
  switch (estado1) {
    ...
    case 2: // derecha por tiempo
      adproporcional();
      if (currentMillis - previousMillis >= 850) {
        pare();
        estado1 = 3;
        previousMillis = currentMillis;
      }
    ...
    case 3: // retrocede hasta tocar switch trasero, corrigiendo
     ...
     rproporcional();
      ...
      if(error < -25){//antes era -15
           digitalWrite(INA3, LOW); digitalWrite(INB3, HIGH);  analogWrite(PWM3, 0);
           digitalWrite(INA2, LOW);  digitalWrite(INB2, HIGH); analogWrite(PWM2, 40);
           digitalWrite(INA1, LOW); digitalWrite(INB1, HIGH); analogWrite(PWM1, 40);
      }
    ...
    case 4: // izquierda por tiempo (corto)
      aiproporcional();
    ...
    case 5: // avanza por el medio + freno por choque
      aproporcional2();
      if (verificarPelotaVioleta()) {
        estado1 = 12;
    ...
    case 7: // retrocede hasta switch trasero
    ...
    case 8://rutina morada del caso 1
      r();
    ...
    case 11:
     gi();
     if (currentMillis - previousMillis >= 100 || error == 0) {
        pare();
        estado1 = 1;
        previousMillis = currentMillis;
      }
    ...
    case 12://rutina morada del caso 5
      r();
      if (currentMillis - previousMillis >= 60) {
        pare();
        estado1 = 13;
    ...
    case 16:
     gi();
     if (currentMillis - previousMillis >= 200 || error == 0) {
        pare();
        estado1 = 5;
        previousMillis = currentMillis;
      }
   ...
   case 18:
    ...
    if (currentMillis - previousMillis >= 1000) { //depende la bateria va a cambiar la velocidad y el tiempo
      pare();
       previousMillis = currentMillis;
       estado1 = 19;
    }
   ...
   case 20:
     aiproporcional();
     if (switchIzquierdoActivado || (currentMillis - previousMillis) >= 900) {
        pare();
        previousMillis = currentMillis;
        estado1 = 21;
      }
   ...
   if (currentMillis - previousMillis >= 600) { //depende la bateria va a cambiar la velocidad y el tiempo
      pare();
       previousMillis = currentMillis;
       estado1 = 22;
    }
