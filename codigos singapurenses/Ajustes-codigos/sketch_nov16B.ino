...
int estadosuc = 0;//ESTADOS DEL SUCCIONOADOR DE PELOTAS
...
void gd (){ // giro derecha
  digitalWrite(INA1, HIGH); digitalWrite(INB1, LOW); analogWrite(PWM1, 20);
  digitalWrite(INA2, HIGH); digitalWrite(INB2, LOW); analogWrite(PWM2, 20);
  digitalWrite(INA3, HIGH); digitalWrite(INB3, LOW); analogWrite(PWM3, 20);
}
     ...
     aproporcional();
      if (verificarPelotaVioleta()){
        if(captura == 0){
              ...
              pare();
              previousMillis = currentMillis; }
        else{//---->aquideberia hacer una rutina de ezquive ya que ya tengo una pelota almacenada
               pare();
               estado1 = 41;
               previousMillis = currentMillis; }
      ...
      if (currentRoll > 7 || currentMillis - previousMillis >= 6000 ){
         pare();
        estado1 = 18;
        previousMillis = currentMillis;
        } 
...
//-----------------------------------------------------------------------
//freno para que al detectar la violeta pare y no siga avanzando.
    case 58:
    //aiproporcional();
    rmax();  // ejecutar movimiento constantemente----->freno magnetico
...
//-----------------------------------------------------------------------
  ...
  if (correccion < -MAX_CORR) correccion = -MAX_CORR;
   ...
   if (verificarPelotaVioleta()){
    ...
    if (currentRoll > 7 || currentMillis - previousMillis >= 8000 ){
      pare();
      estado1 = 19;
      previousMillis = currentMillis;
     }
     break;
    case 19://--freno magnetico
    //aiproporcional();
  ...
  if (correccion < -MAX_CORR) correccion = -MAX_CORR;
   ...
   case 21:
      gi();
         if(error > 85|| currentMillis - previousMillis >= 2000){//----timer
            pare();
            previousMillis = currentMillis;
            estado1 = 22;
                       }
   ...
   if(currentMillis - previousMillis >= 10000){
    ...
    case 24:
    gd();
     if(error < 5|| currentMillis - previousMillis >= 2000){
            pare();
            previousMillis = currentMillis;
            estado1 = 25;
                       }
   ...
   case 31:
      gi();
         if(error > 85|| currentMillis - previousMillis >= 2000){
            pare();
            previousMillis = currentMillis;
            estado1 = 32;
                       }
   ...
   case 41:
      gi();
         if(error > 45|| currentMillis - previousMillis >= 2000){
            pare();
            previousMillis = currentMillis;
            estado1 = 42;
                       }
   ...
   case 43:
   if(currentMillis - previousMillis >= 300){//---------->para identificar el movimiento
    estado1 = 44;
    previousMillis = currentMillis;}
    break;
